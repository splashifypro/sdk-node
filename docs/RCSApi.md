...
async function example() {
  ...
  const body = {
    // object | { to, template_id, variables? }. variables: a list in {{1}}, {{2}} order, or an object keyed by number. 400 missing_variables / variable_too_long when a value is missing or too long.
    body: Object,
...
|------------- | ------------- | ------------- | -------------|
| **body** | `object` | { to, template_id, variables? }. variables: a list in {{1}}, {{2}} order, or an object keyed by number. 400 missing_variables / variable_too_long when a value is missing or too long. | |
...
Send a stored, approved RCS template. Its {{1}}, {{2}} … variables are filled from the variables list, in order. Deprecated alias. POST /rcs/send accepts { to, template_id, variables } directly. Kept for backward-compatibility.
